...
use Ballybran\Helpers\ {
    Http\Hook, Security\Session, Security\Validate, Security\Val
};
...
class Finance extends AbstractController {
    ...
    public function criarBanco() {
        $this->form->post("Cnome")->text()->val("maxlength", 20)->submit();

        $this->model->criarBanco($this->form->getPostData());
        Hook::Header("account/cpanel");
    }
