...
class FuncionarioModel
{
    ...
    public function allPacienteByFuncionario($id) {
        return $this->database->selectManager("SELECT Func_has_Paci.*, Paciente.*, Func_has_Paci.* , usuarios.* FROM Func_has_Paci INNER JOIN Paciente INNER JOIN Funcionarios ON Func_has_Paci.Funcionarios_id = Funcionarios.id AND Func_has_Paci.Paciente_id = Paciente.id INNER JOIN usuarios ON usuarios.id = Paciente.usuarios_id LEFT OUTER JOIN Medicamento ON Medicamento.Paciente_id = Paciente.id  WHERE Medicamento.Funcionarios_id is null  and Funcionarios.usuarios_id = $id" );
    }
    ...
    {
        return $this->database->selectManager(" SELECT Receitas.id FROM Receitas INNER JOIN Medicamento ON Receitas.Medicamento_med_id = Medicamento.med_id INNER JOIN Paciente ON Medicamento.Paciente_id = Paciente.id WHERE Paciente.usuarios_id = $id ORDER BY Receitas.id DESC ");
    }
