...
class FuncionarioModel {
    ...
    public function getPacienteById($id) {
        return $this->database->selectManager(" SELECT   Func_has_Paci.Paciente_id, Func_has_Paci.Funcionarios_id, usuarios.firstname, usuarios.lastname,usuarios.sexo, usuarios.address_1,
        usuarios.address_2, usuarios.city, usuarios.telephone, usuarios.email, usuarios.country FROM Func_has_Paci INNER JOIN Paciente on Func_has_Paci.Paciente_id = Paciente.id INNER JOIN usuarios on Paciente.usuarios_id = usuarios.id WHERE Paciente.usuarios_id =$id ORDER BY Paciente.id DESC");
    }

    public function getPacienteByInternamento($id) {
        return $this->database->selectManager("  SELECT  internamento.id,Func_has_Paci.Paciente_id, Func_has_Paci.Funcionarios_id, usuarios.firstname, usuarios.lastname,usuarios.sexo, usuarios.address_1,
        usuarios.address_2, usuarios.city, usuarios.telephone, usuarios.email, usuarios.country FROM Func_has_Paci INNER JOIN Paciente on Func_has_Paci.Paciente_id = Paciente.id INNER JOIN usuarios on Paciente.usuarios_id = usuarios.id INNER JOIN internamento ON internamento.Paciente_id = Paciente.id WHERE Paciente.usuarios_id =$id ORDER BY Paciente.id DESC ");
         }
    ...
    public function inserirInternamento($data)
    {
        $this->database->insert('internamento', $data);
    }

    public function inserirProblema($data)
    {
        $this->database->insert('ListProblema', $data);
    }
